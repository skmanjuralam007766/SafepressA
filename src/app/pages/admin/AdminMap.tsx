...
export function AdminMap() {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex gap-4">
      {/* Map Area */}
      <div className="flex-1 relative rounded-2xl overflow-hidden bg-white border border-slate-200 dark:bg-slate-800 dark:border-slate-700">
         {/* Uttarakhand Map */}
         <img 
           src={uttarakhandMap} 
           alt="Uttarakhand Map" 
         ...
         {/* Overlay Controls */}
         <div className="absolute top-4 left-4 flex flex-col gap-2 bg-white/95 dark:bg-slate-900/95 p-2 rounded-xl shadow-lg backdrop-blur-sm">
            <Button 
         ...
         {/* Mock Markers - Adjusted position for new map feeling */}
      ...
      {/* Right Panel */}
      <Card className="w-80 flex flex-col h-full dark:bg-slate-800">
        <div className="p-4 border-b border-slate-100 dark:border-slate-700">
            <h3 className="font-semibold text-slate-900 dark:text-white">Live Updates</h3>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="flex gap-3 items-start border-b border-slate-50 dark:border-slate-700 pb-3 last:border-0">
                    ...
                    <div>
                        <p className="text-sm font-medium text-slate-900 dark:text-white">Incident #{2020+i}</p>
                        <div className="flex items-center text-xs text-slate-500 dark:text-slate-400 mt-1">
                            <MapPin className="w-3 h-3 mr-1" /> Sector {i}
                        </div>
                        <div className="flex items-center text-xs text-slate-400 mt-0.5">
                            <Clock className="w-3 h-3 mr-1" /> {i*5}m ago
                        </div>
                    </div>
                </div>
            ))}
        </div>
        <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-b-2xl border-t border-slate-100 dark:border-slate-700">
